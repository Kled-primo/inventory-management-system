...
class PurchaseController extends Controller {
	...
	public function approvedPurchases() {
		$purchases = Purchase::with( array( 'supplier' ) )
			->where( 'status', 3 )->get(); // 1 = approved

		return view(
			'purchases.approved-purchases',
			array(
				'purchases' => $purchases,
			)
		);
	...
	public function edit( $uuid ) {
		...
		// N+1 Problem if load 'createdBy', 'updatedBy',
		$purchase->with( array( 'supplier', 'details' ) )->get();
	...
	public function create() {
		return view(
			'purchases.create',
			array(
				'categories' => Category::where( 'user_id', auth()->id() )->select( array( 'id', 'name' ) )->get(),
				'suppliers'  => Supplier::select( array( 'id', 'name' ) )->get(),
			)
		);
	}
	...
	public function update( Request $request, $uuid ) {
		...
		if ( $request->status == 3 ) {
			...
			foreach ( $products as $product ) {
				...
				->update( array( 'quantity' => DB::raw( 'quantity+' . $product->quantity ) ) );
			}
		}
	...
	public function exportPurchaseReport( Request $request ) {
		...
		$purchases = DB::table( 'purchase_details' )
			...
			->whereBetween( 'purchases.updated_at', array( $sDate, $eDate ) )
